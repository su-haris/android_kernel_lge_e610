...
#if defined(CONFIG_MACH_MSM7X27_U0)
static int flag_lcd_off = 0; 
#endif
...
/*Don't enable esd irq in wq handler as we are powering off panel */	
	...
	int_en_wq_ret = flush_delayed_work_sync(&lcd_esd->esd_int_en_work);
	#else
	int_en_wq_ret = cancel_delayed_work_sync(&lcd_esd->esd_int_en_work);
	#endif
	...
	#else
	panel_power_on_wq_ret = cancel_delayed_work_sync(&lcd_esd->esd_dsi_panel_on);
	#endif
	/* LGE_CHANGE_E jungrock.oh@lge.com 2013-02-02 modify for ESD signal*/
...
#else
	if(gpio_get_value_cansleep(lcd_esd->esd_detect_gpio) == GPIO_U0_LCD_ESD_DETECT_PIN_IS_LOW && (flag_lcd_off == 1)) 
#endif
/* LGE_CHANGE_E jungrock.oh@lge.com 2013-02-02 add for ESD signal*/
	{
		if( 1 == atomic_read(&lcd_esd->esd_irq_state))
